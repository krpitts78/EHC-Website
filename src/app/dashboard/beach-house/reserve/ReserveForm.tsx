...
export function ReserveForm({
  ...
  return (
    <form action={formAction} className="mt-6 space-y-5">
      ...
      <div className="rounded border border-[#a8a395] bg-white/60 px-4 py-3 text-sm">
        <span className="text-[#5a6470]">Reserving as</span>{" "}
        <span className="font-medium">{memberName}</span>
      </div>

      <label className="block text-sm">
        <span className="text-[#5a6470]">Notes for VP Beach House (optional)</span>
        <textarea
          name="notes"
          rows={3}
      ...
      <div className="flex gap-3">
        ...
        <Link
          ...
          className="rounded border border-[#d4c89c]/40 px-5 py-2 text-[#5a6470] hover:border-[#d4c89c] hover:text-[#1a2128]"
        >
          Cancel
        </Link>
      </div>
